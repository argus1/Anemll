...
def main() -> None:
    ...
    for pos, tok in enumerate(token_ids):
        ...
        hidden_states = runner.predict(hidden_states, per_layer_inputs, pos, ctx_len, head_dim)
    ...
    for _ in range(max(args.max_new_tokens - 1, 0)):
        ...
        hidden_states = init_out["hidden_states"]
        per_layer_inputs = init_out.get("per_layer_inputs")
        hidden_states = runner.predict(hidden_states, per_layer_inputs, current_pos, ctx_len, head_dim)
